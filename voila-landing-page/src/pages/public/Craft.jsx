...
const Craft = () => {
  ...
  // Fetch produk dari backend, filter category Craft
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (data.success) {
          const craftProducts = data.data.filter(
            (p) => p.category?.toLowerCase() === "craft",
          );
          setProducts(craftProducts);
        }
      } catch (err) {
